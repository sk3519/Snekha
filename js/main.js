...
const Cart = {
  ...
  add(id, size, qty = 1) {
    ...
    else items.push({ id, size, qty, price: p.price + (p.sizes.find((s) => s.label === size)?.delta || 0), name: p.name, emoji: p.emoji, hue: p.hue, img: p.img });
    Cart.write(items);
...
function cardHTML(p) {
  ...
  <article class="card">
    ...
    <a class="card-media" href="product.html?id=${p.id}" style="background:linear-gradient(135deg,${p.hue.split(",")[0]},${p.hue.split(",")[1]})">
      ...
      ${p.img ? `<img class="card-img" src="${p.img}" alt="${p.name}" loading="lazy" onerror="this.remove()">` : ""}
    </a>
    <div class="card-body">
